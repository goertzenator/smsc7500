...
static unsigned long tx_pending_queue_size = 10UL;
module_param(tx_pending_queue_size, ulong, 0);
MODULE_PARM_DESC(tx_pending_queue_size,"Specifies the size of the tx pending queue lenght");


bool tx_hold_on_completion = TRUE;
	...
	while ((skb = skb_dequeue (&dev->done))) {
		...
		switch (entry->state) {
			...
			if(dev->tx_hold_on_completion)
				...
				if(dev->txq.qlen != 0){
					if (dev->tx_pending_q.qlen >= tx_pending_queue_size){
